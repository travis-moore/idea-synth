...
| `items.decide`                                                                    | Record the **user's** decision (needs `--user-instruction`). Use `respond` with their exact words for anything the agent _asked_ them; accept/qualify/reject only for claims and objections |
