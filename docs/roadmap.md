...
| Codex                                                               | **As a CLI client: yes** (run by the user with Codex, 2026-09-22): a Codex agent drove extract → explore → epistemic → adversarial → synthesis through `bin/synth`, confirming duplicate replay, stale-submission rejection, the review gate, and agent authorship surviving acceptance; separately, the standalone `codex` executable (ChatGPT authentication, API-key variables removed) produced a real extraction that `passes.submit` accepted. **Not implemented:** a Codex provider for web-triggered reasoning                                                                                                           | `tests/agent-api.test.ts` (agent-neutral contract); `tests/codex-findings.test.ts` (the four defects that review found) |
...
Smaller: a Codex provider for web-triggered reasoning (the `codex` executable and its ChatGPT
login have now been seen to work; the adapter boundary is ready); agent-proposed
